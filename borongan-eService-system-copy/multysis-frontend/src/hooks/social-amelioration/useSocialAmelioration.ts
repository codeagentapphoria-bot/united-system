...
const normalizeBeneficiary = (record: BeneficiaryRecord): EnrichedBeneficiary => {
  ...
  return {
    ...
    dateOfBirth: existingFields.dateOfBirth ?? (citizen as any).birthDate ?? citizen.birthdate ?? undefined,
    gender: existingFields.gender ?? citizen.sex ?? undefined,
    phoneNumber: existingFields.phoneNumber ?? citizen.contactNumber ?? (citizen as any).phoneNumber ?? undefined,
    contactNumber: existingFields.contactNumber ?? citizen.contactNumber ?? (citizen as any).phoneNumber ?? undefined,
    address: existingFields.address ?? citizen.streetAddress ?? (citizen as any).address ?? undefined,
    addressRegion: existingFields.addressRegion ?? (citizen as any).addressRegion ?? undefined,
    addressProvince: existingFields.addressProvince ?? (citizen as any).addressProvince ?? undefined,
    addressMunicipality: existingFields.addressMunicipality ?? (citizen as any).addressMunicipality ?? undefined,
    addressBarangay: existingFields.addressBarangay ?? citizen.barangay?.name ?? (citizen as any).addressBarangay ?? undefined,
    addressPostalCode: existingFields.addressPostalCode ?? (citizen as any).addressPostalCode ?? undefined,
    addressStreetAddress: existingFields.addressStreetAddress ?? citizen.streetAddress ?? (citizen as any).addressStreetAddress ?? undefined,
  };
};
