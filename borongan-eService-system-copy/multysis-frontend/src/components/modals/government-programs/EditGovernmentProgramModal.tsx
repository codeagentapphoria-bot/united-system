...
function isRequirementsConfig(raw: unknown): raw is RequirementsConfig {
  return typeof raw === 'object' && raw !== null && !Array.isArray(raw) && 'mode' in raw;
}

const parseRequirements = (raw?: string | null | RequirementsConfig): RequirementsConfig => {
  if (!raw) return DEFAULT_CONFIG;
  if (isRequirementsConfig(raw)) return raw;
  try {
...
interface EditGovernmentProgramModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateGovernmentProgramInput) => Promise<void>;
  ...
  useEffect(() => {
    if (governmentProgram) {
      form.reset({
        ...
        types: governmentProgram.types as GovernmentProgramType[],
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Select
                      ...
                      value={TYPE_OPTIONS.filter(option => field.value?.includes(option.value as GovernmentProgramTypeEnum))}
                      onChange={selected => {
