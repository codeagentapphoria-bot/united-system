...
interface EditPWDModalProps {
  open: boolean;
  onClose: () => void;
  onEdit: (data: PWDInput) => Promise<void>;
  initialData?: any;
}

export const EditPWDModal: React.FC<EditPWDModalProps> = ({
  open,
  onClose,
  onEdit,
  initialData,
      ...
      <DialogContent className={cn("max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0")}>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t bg-white">
          ...
          <Button
            ...
            disabled={form.formState.isSubmitting}
          >
            {form.formState.isSubmitting ? 'Updating...' : 'Update PWD'}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
