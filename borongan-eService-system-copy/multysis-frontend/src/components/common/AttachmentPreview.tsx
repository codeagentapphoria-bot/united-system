...
const Lightbox: React.FC<LightboxProps> = ({ url, label, onClose }) => (
...
export const AttachmentGrid: React.FC<AttachmentGridProps> = ({ attachments }) => {
          ...
          if (isImage) {
            return (
              <button
                key={i}
                type="button"
                onClick={() => setLightbox({ url, label: att.label })}
